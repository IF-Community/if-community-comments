...
export class CommentsController {
  ...
  //Votes
  async handleVote(type: 'add' | 'remove', id: number) {
    const voteComment = await this.model.findOneBy({ comment_id: id });
    await this.model.update(
        { comment_id: id },
        { ...voteComment, vote_points: type == 'add' ? voteComment.vote_points + 1 :  voteComment.vote_points - 1},
    );
    return {
        message: `${type == 'add' ? 'upvote' : 'downvote'} adicionado com sucesso!`
    }
  }


  @Patch('add-upvote/:id')
  async addUpVote(@Param('id') id: number) {
    return this.handleVote('add', id)
  }
  
  @Patch('add-downvote/:id')
  async addDownVote(@Param('id') id: number) {
    return this.handleVote('remove', id)
  }
}
